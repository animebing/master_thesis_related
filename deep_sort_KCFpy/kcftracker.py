...
# KCF tracker
class KCFTracker:
	def __init__(self, track_id, n_init, max_age, feature=None, hog=False, fixed_window=True, multiscale=False):
	...
	def init(self, roi, image):
		...
		#time_4 = time.time()
		#print('get feature time: %.6f(s)' % (time_1 - time_0))
		#print('create gaussian peak: %.f' % (time_2 - time_1))
		#print('create alpha zeros: %.6f' % (time_3 - time_2))
		#print('train: %.6f' % (time_4 - time_3))
	...
	def predict(self, image):
		...
		# time_0 = time.time()
		# tmp_fea = self.getFeatures(image, 0, 1.0)
		# time_1= time.time()
		# print('feature time for detect: %.6f(s)' % (time_1 - time_0))
		# loc, peak_value = self.detect(self._tmpl, tmp_fea)
	...
	def update(self, detection, image):
		...
		self._roi = map(float, detection.tlwh) # to form a list instead of numpy array
	...
	# 	x = self.getFeatures(image, 0, 1.0)
	# 	#time_1 = time.time()
	# 	#print('feature for train: %.6f(s)' % (time_1 - time_0))
	# 	self.train(x, self.interp_factor)
    #
	# 	return self._roi
	...
	# TODO
	def to_tlwh(self):
		return np.asarray(self._roi, dtype=np.float)
