...
def vbb_anno2dict(vbb_file, sub_dir):
    ...
    for frame_id, obj in enumerate(objLists):
        ...
        if len(obj[0]) > 0:
            # convert to xml only if there is annotation
            frame_name = '/'.join([sub_dir, vid_name, 'visible', 'I{:05d}.jpg'.format(frame_id)])
            annos[frame_name] = defaultdict(list)
            annos[frame_name]["id"] = frame_name
            for id, pos, occl, lock, posv in zip(
                ...
                # bingbing: just store frames with person annotation
                if objLbl[id] != 'person':
                    continue
...
def instance2xml_base(anno, img_size, bbox_type='xyxy'):
    ...
    for index, bbox in enumerate(anno['bbox']):
        ...
        # bingbing: a little confused, but it is true
        if bbox_type == 'xyxy':
            xmin, ymin, w, h = bbox
            xmax = xmin+w
            ymax = ymin+h
        ...
        for vbb_file in vbb_files:            
            ...
            if annos:                
                ...
                for filename, anno in sorted(annos.items(), key=lambda x: x[0]):
                    # if "bbox" in anno:
                    # if there is no person annotation in this frame, there is no 'label' key
                    if 'label' not in anno:
                        continue
                    anno_tree = instance2xml_base(anno, IMAGE_SIZE)
                    # bingbing
                    tmp_name = os.path.splitext(filename)[0]
                    tmp_name = tmp_name.replace('visible/', '')
                    outfile = os.path.join(vbb_outputdir, tmp_name + ".xml")
...
if __name__ == "__main__":
    ...
    vbb_inputdir = '/home/bingbing/datasets/kaist/annotations/annotations-vbb'
    xml_outputdir = '/home/bingbing/datasets/kaist/annotations/annotations-xml'
